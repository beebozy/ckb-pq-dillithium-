...
import { CkbRpcClient, PUBLIC_KEY_LEN, SECRET_KEY_LEN, SHANNONS_PER_CKB, WalletError, addressFromPublicKey, buildLockScript, buildSignedTransfer, decodeCkbAddress, generateKeypair, getBalanceSourceInspection, getWalletState, inspectOutPoint, validateTransferRequest, waitForBalance, waitForTransactionConfirmation, } from "./index.js";
...
async function runKeygen(args) {
    ...
    if (network) {
        ...
        console.log(`Then: dillithium-wallet wait --key-file ${quote(outputPath)} --network ${network} --rpc-url <rpc> --indexer-url <indexer>`);
    }
}
async function runAddress(args) {
...
async function runCheckOutpoint(args) {
    const keyFile = requireOption(args, "--key-file");
    const rpcUrl = requireOption(args, "--rpc-url");
    const network = parseNetwork(requireOption(args, "--network"));
    ...
    const diagnosis = inspection.status === "live"
...
async function loadScriptConfig(network) {
...
function parseOptionalCkbAmount(value) {
...
function parseOutPointIndex(value) {
    return toHexQuantity(value);
}
function hexQuantityToBigInt(value) {
    return BigInt(value);
}
function checkOutpointNextSteps(diagnosis) {
...
function hexToBytes(value, filePath, field) {
