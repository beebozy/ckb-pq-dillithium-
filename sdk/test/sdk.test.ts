...
const sampleLock: LockScript = {
  codeHash: `0x${"11".repeat(32)}`,
  hashType: "data2",
  args: `0x${"22".repeat(32)}`,
};
const sampleCellDep: CellDep = {
  outPoint: {
...
function makeLiveCell(capacity: bigint, suffix: string): LiveCell {
  return {
    blockNumber: "0x1",
    outPoint: {
...
function transferOptions(cells: LiveCell[]): BuildTransferOptions {
  return {
    cells,
    fromLock: sampleLock,
    toLock: sampleLock,
...
test("buildWitnessLock rejects malformed lengths", async () => {
  await assert.rejects(
    async () => buildWitnessLock(new Uint8Array(PUBLIC_KEY_LEN - 1), new Uint8Array(SIGNATURE_LEN)),
    /public key must be/,
  );
  await assert.rejects(
    async () => buildWitnessLock(new Uint8Array(PUBLIC_KEY_LEN), new Uint8Array(SIGNATURE_LEN - 1)),
    /signature must be/,
  );
});
...
test("getWalletState reports funded status and minimum capacity", async () => {
  ...
  const state = await getWalletState(client as never, keypair.publicKey, {
    codeHash: sampleLock.codeHash,
    hashType: sampleLock.hashType,
    cellDep: sampleCellDep,
    network: "testnet",
...
test("inspectOutPoint reports a live matching cell", async () => {
  ...
  const inspection = await inspectOutPoint(client as never, sampleOutPoint, sampleLock);
...
test("inspectOutPoint reports dead and unknown outpoints without cell data", async () => {
  ...
  const dead = await inspectOutPoint(deadClient as never, sampleOutPoint, sampleLock);
  const unknown = await inspectOutPoint(unknownClient as never, sampleOutPoint, sampleLock);
...
test("getBalanceSourceInspection reports indexer lag or unavailability", async () => {
  ...
  const lagged = await getBalanceSourceInspection(laggedClient as never);
  const unavailable = await getBalanceSourceInspection(unavailableClient as never);
  const aligned = await getBalanceSourceInspection(alignedClient as never);
...
test("waitForBalance resolves after funding appears", async () => {
  ...
  const result = await waitForBalance(client as never, keypair.publicKey, {
    codeHash: sampleLock.codeHash,
    hashType: sampleLock.hashType,
    cellDep: sampleCellDep,
    network: "testnet",
...
test("waitForBalance times out with a wallet error", async () => {
  ...
  await assert.rejects(
    async () => waitForBalance(client as never, keypair.publicKey, {
      codeHash: sampleLock.codeHash,
      hashType: sampleLock.hashType,
      cellDep: sampleCellDep,
      network: "testnet",
    ...
    (error: unknown) => error instanceof WalletError && error.code === "balance_wait_timeout",
  );
});
...
test("waitForTransactionConfirmation resolves when a committed status appears", async () => {
  ...
  const result = await waitForTransactionConfirmation(client as never, {
    txHash: "0xabc",
    timeoutMs: 50,
    intervalMs: 0,
...
test("waitForTransactionConfirmation times out with a wallet error", async () => {
  ...
  await assert.rejects(
    async () => waitForTransactionConfirmation(client as never, {
      txHash: "0xdead",
      timeoutMs: 0,
      intervalMs: 0,
      acceptedStatuses: ["committed"],
    }),
    (error: unknown) => error instanceof WalletError && error.code === "transaction_confirmation_timeout",
  );
});

test("validateTransferRequest rejects recipient network mismatch", () => {
  assert.throws(
    () => validateTransferRequest(transferOptions([makeLiveCell(100n * SHANNONS_PER_CKB, "11")]), "mainnet"),
    (error: unknown) => error instanceof WalletError && error.code === "invalid_recipient_network",
  );
});
...
test("validateTransferRequest rejects amounts below minimum capacity", () => {
  assert.throws(
    ...
    (error: unknown) => error instanceof WalletError && error.code === "amount_below_minimum_capacity",
  );
});
...
test("validateTransferRequest rejects insufficient total capacity", () => {
  assert.throws(
    ...
    (error: unknown) => error instanceof WalletError && error.code === "insufficient_total_capacity",
  );
});
...
test("validateTransferRequest rejects transfers that cannot cover fee or valid change", () => {
  assert.throws(
    ...
    (error: unknown) => error instanceof WalletError && error.code === "insufficient_capacity_for_fee",
  );
});
